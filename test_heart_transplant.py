import pytest
from heart_transplant import ISHLTBiopsyScorerAgent, DSATiterTrackerAgent, ImmunosuppressionTroughAuditorAgent, TransplantCoordinator, main


def test_sub_agents():
    a1 = ISHLTBiopsyScorerAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = DSATiterTrackerAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = ImmunosuppressionTroughAuditorAgent()
    alerts3 = a3.evaluate({"status_text": "DISCORDANT_FINDING"})
    assert len(alerts3) == 1


def test_coordinator():
    coord = TransplantCoordinator()
...
def test_domain_registry():
    from heart_transplant import DomainKnowledgeRegistry
